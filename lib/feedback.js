...
(function () {
  ...
  function buildUI() {
    ...
    root.innerHTML = [
      ...
      '    </div>',
      '    <div class="cf-publish-box" id="cf-publish-box">',
      '      <div>',
      '        <strong>GitHub Pages</strong>',
      '        <span id="cf-publish-status">checking...</span>',
      '      </div>',
      '      <button id="cf-publish" class="cf-btn cf-btn-small" disabled>publish</button>',
      '    </div>',
  ...
  function renderPublishStatus(data) {
    ...
    const button = $("cf-publish");
    const status = $("cf-publish-status");
    const box = $("cf-publish-box");
    if (!button || !status || !box) return;

    const state = publishInFlight ? "publishing" : (publishStatus && publishStatus.state) || "unknown";
    box.dataset.state = state;
    button.disabled = publishInFlight || !publishStatus || !publishStatus.can_publish;
    button.textContent = publishInFlight ? "publishing..." : "publish";

    if (publishInFlight) {
      status.textContent = "publishing static copy and pushing";
    } else if (!publishStatus || !publishStatus.ok) {
      status.textContent = "publish status unavailable";
    } else {
      status.textContent = publishStatus.message || "publish status unknown";
    }
  }
  ...
  function bindEvents() {
    ...
    $("cf-publish").addEventListener("click", publishNow);
