...
(function () {
  ...
  const PUBLISH_STATUS_URL = "/publish/status";
  const PUBLISH_URL = "/publish";
  const POLL_INTERVAL_MS = 4000;
  const PUBLISH_POLL_INTERVAL_MS = 8000;
  const OUTER_HTML_MAX = 600;
  const TEXT_SNIPPET_MAX = 220;
  const ACTIVE_TAB_KEY = "cf-active-tab-panel";
  ...
  function buildUI() {
    ...
    root.innerHTML = [
      ...
      '  <button id="cf-publish-main" class="cf-btn cf-publish-launch cf-publish-action" disabled title="Checking publish status">',
      '    publish',
      '  </button>',
      ...
      '    </div>',
      '    <div class="cf-publish-box cf-publish-surface" id="cf-publish-box">',
      '      <div>',
      '        <strong>GitHub Pages</strong>',
      '        <span id="cf-publish-status" class="cf-publish-status">checking...</span>',
      '      </div>',
      '      <button id="cf-publish" class="cf-btn cf-btn-small cf-publish-action" disabled>publish</button>',
      '    </div>',
  ...
  function renderPublishStatus(data) {
    ...
    const buttons = Array.from(document.querySelectorAll(".cf-publish-action"));
    const statuses = Array.from(document.querySelectorAll(".cf-publish-status"));
    const surfaces = Array.from(document.querySelectorAll(".cf-publish-surface"));
    if (!buttons.length) return;

    const state = publishInFlight ? "publishing" : (publishStatus && publishStatus.state) || "unknown";
    const disabled = publishInFlight || !publishStatus || !publishStatus.can_publish;
    surfaces.forEach((surface) => { surface.dataset.state = state; });
    buttons.forEach((button) => {
      button.dataset.state = state;
      button.disabled = disabled;
      button.textContent = publishInFlight ? "publishing..." : "publish";
    });

    let message = "publish status unavailable";
    if (publishInFlight) {
      message = "publishing this explainer to GitHub Pages";
    } else if (!publishStatus || !publishStatus.ok) {
      message = "publish status unavailable";
    } else {
      message = publishStatus.message || "publish status unknown";
    }
    statuses.forEach((status) => { status.textContent = message; });
    buttons.forEach((button) => { button.title = message; });
  }

  async function fetchPublishStatus() {
    try {
      const resp = await fetch(PUBLISH_STATUS_URL + "?t=" + Date.now());
      if (!resp.ok) throw new Error("server returned " + resp.status);
      renderPublishStatus(await resp.json());
    } catch (e) {
      renderPublishStatus({ ok: false, state: "error", can_publish: false, message: "publish server unavailable" });
    }
  }

  async function publishNow() {
    if (publishInFlight || !publishStatus || !publishStatus.can_publish) return;
    ...
    try {
      ...
      showToast(data.message || "published", 4500);
      renderPublishStatus(data.status);
      await fetchPublishStatus();
    } catch (e) {
      showToast("publish failed: " + e.message, 6500);
      await fetchPublishStatus();
    } finally {
  ...
  function setPendingReload(addCount, targets = []) {
    pendingReloadCount += addCount;
    pendingReloadTargets = pendingReloadTargets.concat(targets.filter(Boolean));
  ...
  function doReload() {
    ...
    const target = pendingReloadTargets[pendingReloadTargets.length - 1];
    if (target) {
      sessionStorage.setItem("cf-focus-change-id", target);
  ...
  function onHistoryUpdated() {
    ...
    if (missing.length > 0) {
      const missingIds = missing.map(ch => changeIdFromAnchor(ch.anchor || ch.id)).filter(Boolean).sort().join("|");
      sessionStorage.setItem("cf-last-reload-anchors", missingIds);
    } else {
      sessionStorage.removeItem("cf-last-reload-anchors");
    }
    setPendingReload(trulyNew.length, trulyNew.map(ch => changeIdFromAnchor(ch.anchor || ch.id)));
  }
  ...
  function changeIdFromAnchor(anchor) {
    if (!anchor) return "";
    const raw = String(anchor).trim();
    const match = raw.match(/data-cf-change\s*=\s*["']([^"']+)["']/);
  ...
  function findAnchorNode(anchor) {
    ...
    if (raw.startsWith("[") || raw.startsWith("#") || raw.startsWith(".")) {
      ...
      } catch { /* not a selector; try as id below */ }
    }
    const id = changeIdFromAnchor(raw);
    if (!id) return null;
    // Use ~= (whitespace-separated word match) so an element can carry multiple
    // anchors at once, e.g. data-cf-change="ch-foo ch-bar".
    return document.querySelector(`[data-cf-change~="${CSS.escape(id)}"]`);
  }
  ...
  function persistActiveTabPanel() {
    const id = activeTabPanelId();
    if (id) sessionStorage.setItem(ACTIVE_TAB_KEY, id);
  }

  function findTabButton(tabName, panelId) {
    const candidates = Array.from(document.querySelectorAll(".tab-btn, [onclick*='switchTab'], [data-tab]"));
    return candidates.find((btn) => {
      const onclick = btn.getAttribute("onclick") || "";
      const dataTab = btn.dataset.tab || "";
      return dataTab === tabName || dataTab === panelId || onclick.includes(`'${tabName}'`) || onclick.includes(`"${tabName}"`);
    }) || null;
  }

  function revealContainingTab(node) {
    const panel = node && node.closest ? node.closest(".tab-panel[id]") : null;
    if (!panel) return false;
    if (panel.classList.contains("active")) {
      persistActiveTabPanel();
      return false;
    }

    const tabName = panel.id.replace(/^tab-/, "");
    const btn = findTabButton(tabName, panel.id);
    let usedPageSwitch = false;
    if (typeof window.switchTab === "function") {
      try {
        window.switchTab(tabName, btn || undefined);
        usedPageSwitch = true;
      } catch { /* fall back to generic tab activation */ }
    }
  ...
  function restoreActiveTabPanel() {
    const hashId = decodeURIComponent((location.hash || "").replace(/^#/, ""));
    const hashNode = hashId ? findAnchorNode(hashId) : null;
    if (hashNode) {
      revealContainingTab(hashNode);
      return;
    }
    const saved = sessionStorage.getItem(ACTIVE_TAB_KEY);
    const panel = saved ? document.getElementById(saved) : null;
    if (panel) revealContainingTab(panel);
  }

  function setChangeHash(anchor) {
    const id = changeIdFromAnchor(anchor);
    if (!id) return;
    window.history.replaceState(null, "", "#" + encodeURIComponent(id));
  }

  function focusHashChange() {
    const saved = sessionStorage.getItem("cf-focus-change-id");
    if (saved) sessionStorage.removeItem("cf-focus-change-id");
    const hash = decodeURIComponent((location.hash || "").replace(/^#/, ""));
    const target = saved || hash;
    if (target && findAnchorNode(target)) focusChange({ id: target, anchor: target });
  }
  ...
  function focusChange(ch) {
    ...
    const switched = revealContainingTab(node);
    setChangeHash(anchor);
    ...
    if (switched) setTimeout(scrollToChange, 100);
    else scrollToChange();
  }
  ...
  function bindEvents() {
    ...
    document.querySelectorAll(".cf-publish-action").forEach((button) => {
      button.addEventListener("click", publishNow);
    });
    ...
    window.addEventListener("hashchange", () => setTimeout(focusHashChange, 0));
    ...
    document.addEventListener("click", (e) => {
      const tabControl = e.target.closest && e.target.closest(".tab-btn, [onclick*='switchTab']");
      if (tabControl && !insideOurUI(tabControl)) setTimeout(persistActiveTabPanel, 0);
    }, true);
  ...
  function init() {
    ...
    fetchHistory().then(() => {
      if (shouldAutoTour && flattenChanges().length > 0) {
        ...
        setTimeout(focusHashChange, 250);
      }
    });
    pollTimer = setInterval(fetchHistory, POLL_INTERVAL_MS);
    fetchPublishStatus();
    publishTimer = setInterval(fetchPublishStatus, PUBLISH_POLL_INTERVAL_MS);
